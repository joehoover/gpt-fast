...
def multinomial_sample_one_no_sync(
    ...
    Instead of using torch.multinomial, this implementation uses the Gumbel-Max trick,
    which is a method for efficiently sampling from a categorical distribution.
    """

    q = torch.empty_like(probs_sort).exponential_(1)
    # We can simulate a sample from Gumbel(0, 1) by sampling from an exponential distribution with rate=1
    # Normally, we would add these to the log probabilities, but since we're taking the argmax, we can skip this step.

    return torch.argmax(probs_sort / q, dim=-1, keepdim=True).to(dtype=torch.int)
    # The argmax of the log probabilities divided by the Gumbel noise is the same as the argmax of the original probabilities.


def logits_to_probs(logits, temperature: float = 1.0, top_k: Optional[int] = None):
    """
    Convert logits to probabilities via softmax with optional temperature scaling and top-k filtering.
    """
